...
" open current file/line in github on master/main branch
...
" open blame for current file/line in github on master/main branch
command! GHB echo system("open https://$(git remote -v  | sed -rn -e '1s#.*@(.*:.*)\\.git.*#\\1#' -e '1s#:#/#p')/blame/$(if git show-ref --verify --quiet refs/heads/master; then echo master; else echo main; fi)/$(git -C " . shellescape(expand('%:p:h')) . " ls-files --full-name " . expand('%:t') . ")#L" . line('.'))
